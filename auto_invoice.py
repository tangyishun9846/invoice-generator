...
"""
商业发票自动生成器
自动扫描当前文件夹（或指定文件夹）里的源文件 (PI / 出口许可证 / 配载),
提取数据后生成商业发票 PDF, 文件名带当天日期.

用法:
    python3 auto_invoice.py
...
def find_source_files(folder: Path):
    """在 folder 及其一层子目录中识别 PI / 出口许可证 / 配载 文件"""
...
def parse_pi(text: str, consignee_hint: str = "") -> dict:
    ...
    # 收货人地址: 用收货人公司名作为锚点, 取后面的几行直到 INCOTERMS / NTN 块结束
    if consignee_hint:
        # 匹配 consignee 公司名出现的位置
...
def parse_license(text: str) -> dict:
    ...
    # 合同签订日期: 找"20\d{6}"且前后不是数字 (避免匹配长 ID 内的子串)
    for m in re.finditer(r"(?<!\d)(20\d{6})(?!\d)", text):
        try:
            dt = datetime.strptime(m.group(1), "%Y%m%d")
            # 合理性检查: 年份在 2000-2099
            if 2000 <= dt.year <= 2099 and dt.year != datetime.now().year + 100:
                d["contract_date"] = dt.strftime("%b,%d,%Y").upper()
                break
        except ValueError:
            pass

    # 数量: 找所有 *<num> 模式, 取最大值作为数量 (单位是千克)
    nums = []
...
def format_port(raw: str, default_country: str = "PAKISTAN") -> tuple:
    """把 'KARACHI PAKISTAN' 这种格式拆成 ('KARACHI PORT, PAKISTAN', 'KARACHI')

    返回 (完整名, 城市名) 用于 Port of Discharge / INCOTERMS"""
    raw = raw.strip()
    if not raw:
...
<table class="goods">
  ...
  <tr class="other">
    <td colspan="5">
      ...
      <p>&nbsp;</p>
      <p>CFR {d['discharge_city']} SEAPORT, {d['port_discharge'].split(',')[-1].strip() if ',' in d['port_discharge'] else 'PAKISTAN'}</p>
      <p>{pi_line}</p>
      <p>{pi_date_line}</p>
...
def main():
    ...
    # 先解析许可证拿到收货人姓名, 再用它去 PI 里精准定位地址
    lic = parse_license(pdf_text(sources["license"]))
    pi = parse_pi(pdf_text(sources["pi"]), consignee_hint=lic.get("consignee_name", ""))
    bk = parse_booking(pdf_text(sources["booking"]))

    print("\n📊 提取的数据:")
    print(f"  PI No: {pi.get('pi_no')}, PI Date: {pi.get('pi_date')}")
    print(f"  收货人: {lic.get('consignee_name')}")
    print(f"  地址: {pi.get('consignee_addr', '').replace(chr(10), ' / ')}")
    print(f"  合同号: {lic.get('contract_no')}, 合同日期: {lic.get('contract_date')}")
    print(f"  起运港: {bk.get('port_loading')}, 卸货港: {bk.get('port_discharge_raw')}")
    print(f"  数量: {lic.get('qty_mt')}MT, 单价: USD {pi.get('unit_price')}/{pi.get('price_unit')}")
    ...
    # 加载印章
    sig_b64 = base64.b64encode((folder / "安然签名章.png").read_bytes()).decode()
    seal_b64 = base64.b64encode((folder / "公司名称章.png").read_bytes()).decode()

    html = build_html(data, sig_b64, seal_b64)
    html_path = folder / ".invoice_temp.html"
    html_path.write_text(html, encoding="utf-8")

    out_pdf = folder / f"商业发票_{data['invoice_no']}_修改于{today.strftime('%Y%m%d')}.pdf"
    if out_pdf.exists():
        out_pdf.unlink()
